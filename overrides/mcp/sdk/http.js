...
async function handleStreamable(serverBackendFactory, req, res, sessions) {
  ...
  if (sessionId) {
    ...
    // Best-effort recovery: if the server restarted, the client might keep using a stale
    // Mcp-Session-Id and never re-send initialize. Create a compat session so existing clients
    // can continue without restarting.
    const recovered = await recoverStreamableSession(serverBackendFactory, String(sessionId), sessions);
...
async function recoverStreamableSession(serverBackendFactory, sessionId, sessions) {
  testDebug(`recover http session: ${sessionId}`);
  const backend = serverBackendFactory.create();
  if (backend.initialize) {
    const cwd = process.cwd();
    const clientInfo = {
      name: "playwright-mcp-session-recovery",
      version: "0.0.0",
      roots: [{ uri: (0, import_url.pathToFileURL)(cwd).href }],
      timestamp: Date.now()
    };
    await backend.initialize(clientInfo);
  }

  const server = mcpServer.createServer(serverBackendFactory.name, serverBackendFactory.version, backend, true);
  // Recreate a streamable session that accepts the client-provided session id.
  // This keeps long-lived clients working across server restarts.
  const transport = new mcpBundle.StreamableHTTPServerTransport({
